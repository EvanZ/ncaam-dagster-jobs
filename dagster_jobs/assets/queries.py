...
def insert_table_stage_top_lines(date: str, women: bool=False) -> str:
    ...
    with lines as (
        ...
        select
            ...
            round(
                0.6*(orb/(coalesce(ta.orb_ortg,1.0)*coalesce(oa.orb_drtg,1.0))) 
                + 0.3*(drb/(coalesce(ta.drb_drtg,1.0)*coalesce(oa.drb_ortg,1.0))) 
                + 1.0*(stl/(coalesce(ta.stl_drtg,1.0)*coalesce(oa.stl_ortg,1.0)))
                + 0.7*(blk/(coalesce(ta.blk_drtg,1.0)*coalesce(oa.blk_ortg,1.0))) 
                + 0.3*(ast_dunk/(coalesce(ta.ast_dunk_ortg,1.0)*coalesce(oa.ast_dunk_drtg,1.0)) 
                    + ast_mid/(coalesce(ta.ast_mid_ortg,1.0)*coalesce(oa.ast_mid_drtg,1.0)) 
                    + ast_layup/(coalesce(ta.ast_layup_ortg,1.0)*coalesce(oa.ast_layup_drtg,1.0))) 
                + 1.0*(unast_dunk/(coalesce(ta.unast_dunk_ortg,1.0)*coalesce(oa.unast_dunk_drtg,1.0)) 
                    + unast_layup/(coalesce(ta.unast_layup_ortg,1.0)*coalesce(oa.unast_layup_drtg,1.0)) 
                    + unast_mid/(coalesce(ta.unast_mid_ortg,1.0)*coalesce(oa.unast_mid_drtg,1.0))
                    + unast_tip) 
                + 2.0*unast_3pt/(coalesce(ta.unast_3pt_ortg,1.0)*coalesce(oa.unast_3pt_drtg,1.0)) 
                + 1.3*ast_3pt /(coalesce(ta.ast_3pt_ortg,1.0)*coalesce(oa.ast_3pt_drtg,1.0))
                + (-0.7)*(miss_tip  
                    + miss_dunk/(coalesce(ta.miss_dunk_ortg,1.0)*coalesce(oa.miss_dunk_drtg,1.0))
                    + miss_layup/(coalesce(ta.miss_layup_ortg,1.0)*coalesce(oa.miss_layup_drtg,1.0))
                    + miss_mid/(coalesce(ta.miss_mid_ortg,1.0)*coalesce(oa.miss_mid_drtg,1.0))
                    + miss_3pt/(coalesce(ta.miss_3pt_ortg,1.0)*coalesce(oa.miss_3pt_drtg,1.0))) 
                + 0.5*ftm
                - 0.5*(fta-ftm) 
                + 0.1*fta / (coalesce(ta.fta_ortg,1.0)*coalesce(oa.fta_drtg,1.0))
                + 0.1*(fga/(coalesce(ta.fga_ortg,1.0)*coalesce(oa.fga_drtg,1.0)) 
                    - fg3a/(coalesce(ta.fg3a_ortg,1.0)*coalesce(oa.fg3a_drtg,1.0))) 
                + 0.2*fg3a / (coalesce(ta.fg3a_ortg,1.0)*coalesce(oa.fg3a_drtg,1.0))
                + 0.7*ast / (coalesce(ta.ast_ortg,1.0)*coalesce(oa.ast_drtg,1.0))
                - 0.8*(tov/(coalesce(ta.tov_ortg,1.0)*coalesce(oa.tov_drtg,1.0)))
            , 3) as ez,
            struct_pack(
                rebounding:=0.6*(orb/(coalesce(ta.orb_ortg,1.0)*coalesce(oa.orb_drtg,1.0))) 
                + 0.3*(drb/(coalesce(ta.drb_drtg,1.0)*coalesce(oa.drb_ortg,1.0))),
                scoring:=0.3*(ast_dunk/(coalesce(ta.ast_dunk_ortg,1.0)*coalesce(oa.ast_dunk_drtg,1.0)) 
                    + ast_mid/(coalesce(ta.ast_mid_ortg,1.0)*coalesce(oa.ast_mid_drtg,1.0)) 
                    + ast_layup/(coalesce(ta.ast_layup_ortg,1.0)*coalesce(oa.ast_layup_drtg,1.0))) 
                + 1.0*(unast_dunk/(coalesce(ta.unast_dunk_ortg,1.0)*coalesce(oa.unast_dunk_drtg,1.0)) 
                    + unast_layup/(coalesce(ta.unast_layup_ortg,1.0)*coalesce(oa.unast_layup_drtg,1.0)) 
                ...
                + 0.1*(fga/(coalesce(ta.fga_ortg,1.0)*coalesce(oa.fga_drtg,1.0)) 
                    - fg3a/(coalesce(ta.fg3a_ortg,1.0)*coalesce(oa.fg3a_drtg,1.0))) 
                + 0.2*fg3a / (coalesce(ta.fg3a_ortg,1.0)*coalesce(oa.fg3a_drtg,1.0)),
                passing:=0.7*ast / (coalesce(ta.ast_ortg,1.0)*coalesce(oa.ast_drtg,1.0))
                - 0.8*(tov/(coalesce(ta.tov_ortg,1.0)*coalesce(oa.tov_drtg,1.0))),
                stocks:=1.0*(stl/(coalesce(ta.stl_drtg,1.0)*coalesce(oa.stl_ortg,1.0)))
                + 0.7*(blk/(coalesce(ta.blk_drtg,1.0)*coalesce(oa.blk_ortg,1.0)))
            ) as ez_components
        from lines l 
